...
const stream_indices = new Guacamole.IntegerPool();
...
export function Client(tunnel: Tunnel) {
  ...
  this.importState = function importState(state, callback) {
    ...
    for (key in state.layers) {
      ...
      if (index > 0 && importLayer.parent >= 0) {
        ...
        const parent = getLayer(importLayer.parent);
        ...
        const matrix = importLayer.matrix;
  ...
  this.sendTouchState = function sendTouchState(touchState, applyDisplayScale) {
    ...
    const x = touchState.x;
    const y = touchState.y;
  ...
  this.createOutputStream = function createOutputStream() {
    ...
    // Allocate index
    const index = stream_indices.next();

    // Return new stream
    const stream = output_streams[index] = new Guacamole.OutputStream(guac_client, index);
    return stream;
  ...
  this.createAudioStream = function (mimetype) {
    ...
    const stream = guac_client.createOutputStream();
   ...
   */
  const getLayer = function getLayer(index) {
   ...
   *     with this client.
   */
  const getLayerIndex = function getLayerIndex(layer) {
  ...
  const instructionHandlers = {
    ...
    "ack": function (parameters) {
      ...
      const code = parseInt(parameters[2]);

      // Get stream
      const stream = output_streams[stream_index];
      if (stream) {
    ...
    "body": function handleBody(parameters) {
      ...
      // Get object
      const objectIndex = parseInt(parameters[0]);
      const object = objects[objectIndex];

      const streamIndex = parseInt(parameters[1]);
      const mimetype = parameters[2];
      const name = parameters[3];

      // Create stream if handler defined
      if (object && object.onbody) {
        const stream = streams[streamIndex] = new Guacamole.InputStream(guac_client, streamIndex);
    ...
    "cfill": function (parameters) {
      ...
      const r = parseInt(parameters[2]);
      const g = parseInt(parameters[3]);
      const b = parseInt(parameters[4]);
      const a = parseInt(parameters[5]);
    ...
    "copy": function (parameters) {
      ...
      const srcX = parseInt(parameters[1]);
      ...
      const dstX = parseInt(parameters[7]);
    ...
    "cstroke": function (parameters) {
      ...
      const channelMask = parseInt(parameters[0]);
      const layer = getLayer(parseInt(parameters[1]));
      const cap = lineCap[parseInt(parameters[2])];
      const join = lineJoin[parseInt(parameters[3])];
      const thickness = parseInt(parameters[4]);
      const r = parseInt(parameters[5]);
      const g = parseInt(parameters[6]);
      const b = parseInt(parameters[7]);
      const a = parseInt(parameters[8]);
    ...
    "cursor": function (parameters) {
      ...
      const srcX = parseInt(parameters[3]);
    ...
    "error": function (parameters) {
      ...
      const code = parseInt(parameters[1]);
    ...
    "filesystem": function handleFilesystem(parameters) {
      ...
      if (guac_client.onfilesystem) {
        const object = objects[objectIndex] = new Guacamole.Object(guac_client, objectIndex);
    ...
    "lfill": function (parameters) {
      ...
      const channelMask = parseInt(parameters[0]);
      const layer = getLayer(parseInt(parameters[1]));
      const srcLayer = getLayer(parseInt(parameters[2]));
    ...
    "move": function (parameters) {
      ...
      const layer_index = parseInt(parameters[0]);
      const parent_index = parseInt(parameters[1]);
      const x = parseInt(parameters[2]);
      const y = parseInt(parameters[3]);
      const z = parseInt(parameters[4]);

      // Only valid for non-default layers
      if (layer_index > 0 && parent_index >= 0) {
        const layer = getLayer(layer_index);
        const parent = getLayer(parent_index);
    ...
    "nest": function (parameters) {
      const parser = getParser(parseInt(parameters[0]));
    ...
    "set": function (parameters) {
      ...
      const handler = layerPropertyHandlers[name];
      if (handler)
        handler(layer, value);
    ...
    "sync": function (parameters) {
      ...
      const timestamp = parseInt(parameters[0]);
